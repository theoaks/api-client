import {
    isObject, isEmpty, trim, trimEnd, trimStart,
    defaultsDeep, merge
} from "lodash";
...
export class ApiClient {
    ...
    buildUrl(module, endpoint, query = undefined) {
        let url = new URL(
            `${trim(module, '/')}/${trimStart(endpoint, '/')}`,
            this._baseUrl
        );
    ...
    send(module, endpoint, options) {
        ...
        let query
            = options.method.toLocaleLowerCase() === HTTP_METHODS.get.toLowerCase()
            ? merge(options.data || {}, options.query || {})
            : options.query;
        let body = options.method.toLocaleLowerCase() === HTTP_METHODS.get.toLowerCase()
            ? null : options.data;
